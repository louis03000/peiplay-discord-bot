...
TW_TZ = timezone(timedelta(hours=8))  # 台灣時區設定
...
@bot.tree.command(name="lock_general", description="鎖定 general 頻道，只允許管理員發言")
...
async def lock_general(interaction: discord.Interaction):
    ...
    if not general_channel:
        await interaction.response.send_message("❌ 找不到名為 #general 的頻道。", ephemeral=True)
        return
    ...
    await interaction.response.send_message("✅ 已鎖定 #general 頻道，僅限管理員發言。", ephemeral=True)

@bot.tree.command(name="unlock_general", description="解鎖 general 頻道，允許所有人發言")
@app_commands.checks.has_permissions(administrator=True)
async def unlock_general(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("✅ 已解鎖 #general 頻道，所有人可發言。", ephemeral=True)
